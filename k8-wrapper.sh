...
# k8 (GLM-4.5) wrapper script for debate consensus system
# Routes through OpenRouter proxy with full MCP tool access
# Open-source backup option for vendor independence

export CLAUDE_CONFIG_DIR="$HOME/.claude-k8"
export ANTHROPIC_BASE_URL="http://localhost:3464"
export ANTHROPIC_API_KEY="k8-debate-key"

# Ensure proxy server is running
if ! curl -s http://localhost:3464/health > /dev/null 2>&1; then
    echo "Error: k8 proxy server not running. Start it with: node $(dirname "$0")/k-proxy-server.js" >&2
    exit 1
fi

# Ensure MCP config exists
if [ ! -f "$CLAUDE_CONFIG_DIR/.claude.json" ]; then
    echo "Error: k8 configuration not found at $CLAUDE_CONFIG_DIR/.claude.json" >&2
    exit 1
fi
