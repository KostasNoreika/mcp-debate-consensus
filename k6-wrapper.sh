...
# k6 (GPT-5 Max Thinking) wrapper script for debate consensus system
# Routes through OpenRouter proxy with full MCP tool access
# Configured for maximum reasoning capability with 128K output tokens

export CLAUDE_CONFIG_DIR="$HOME/.claude-k6"
export ANTHROPIC_BASE_URL="http://localhost:3462"
export ANTHROPIC_API_KEY="k6-debate-key"

# Ensure proxy server is running
if ! curl -s http://localhost:3462/health > /dev/null 2>&1; then
    echo "Error: k6 proxy server not running. Start it with: node $(dirname "$0")/k-proxy-server.js" >&2
    exit 1
fi

# Ensure MCP config exists
if [ ! -f "$CLAUDE_CONFIG_DIR/.claude.json" ]; then
    echo "Error: k6 configuration not found at $CLAUDE_CONFIG_DIR/.claude.json" >&2
    exit 1
fi
